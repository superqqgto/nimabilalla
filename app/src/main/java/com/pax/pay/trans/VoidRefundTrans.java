...
public class VoidRefundTrans extends BaseTrans {
    ...
    public VoidRefundTrans() {
        super(ETransType.VOIDREFUND, null);
        isNeedFindOrigTrans = true;
        isNeedInputTransNo = true;
    }
    public VoidRefundTrans(TransData origTransData, TransEndListener transListener) {
        super(ETransType.VOIDREFUND, transListener);
        this.origTransData = origTransData;
        isNeedFindOrigTrans = false;
        isNeedInputTransNo = false;
    }

    public VoidRefundTrans(String origTransNo, TransEndListener transListener) {
        super(ETransType.VOIDREFUND, transListener);
    ...
    protected void bindStateOnAction() {
        ActionInputPassword inputPasswordAction = new ActionInputPassword(new AAction.ActionStartListener() {
            ...
            public void onStart(AAction action) {
                ((ActionInputPassword) action).setParam(6,
                        ContextUtils.getString(R.string.prompt_void_refund_pwd), null);
            }
        });
        bind(VoidRefundTrans.State.INPUT_PWD.toString(), inputPasswordAction);

        ActionInputTransData enterTransNoAction = new ActionInputTransData(new AAction.ActionStartListener() {

            @Override
            public void onStart(AAction action) {
                ((ActionInputTransData) action).setParam(ContextUtils.getString(R.string.trans_void_refund))
                        .setInputLine1(ContextUtils.getString(R.string.prompt_input_transno), EInputType.NUM, 6, true);
            }
        }, 1);
        bind(VoidRefundTrans.State.ENTER_TRANSNO.toString(), enterTransNoAction);
        ...
        bind(VoidRefundTrans.State.ONLINE_DCC.toString(), transOnlineAction_dcc);
...
//                DccTransData dccTransData = transData.getDccTransData();
                String transName = ContextUtils.getString(R.string.trans_void_refund);
...
//                map.put(ContextUtils.getString(R.string.DCC_TransType), ContextUtils.getString(R.string.trans_dcc));
        ...
        bind(VoidRefundTrans.State.ENQUIRE.toString(), EnquireAction);
...
// confirm information
        ActionDispTransDetail confirmInfoAction = new ActionDispTransDetail(new AAction.ActionStartListener() {
            ...
            public void onStart(AAction action) {
                ...
                ((ActionDispTransDetail) action).setParam(ContextUtils.getString(R.string.trans_void_refund), map);
            }
        });
        bind(VoidRefundTrans.State.TRANS_DETAIL.toString(), confirmInfoAction);
        ...
        bind(VoidRefundTrans.State.ONLINE.toString(), transOnlineAction);
        ...
        bind(VoidRefundTrans.State.SIGNATURE.toString(), signatureAction);

        //offline send
        ActionOfflineSend offlineSendAction = new ActionOfflineSend(null);
        bind(VoidRefundTrans.State.OFFLINE_SEND.toString(), offlineSendAction);
        ...
        bind(VoidRefundTrans.State.PRINT_PREVIEW.toString(), printPreviewAction);
        // print action
        ...
        bind(VoidRefundTrans.State.PRINT_TICKET.toString(), printTransReceiptAction);
        ...
        if (SpManager.getSysParamSp().get(SysParamSp.OTHTC_VERIFY).equals(SysParamSp.Constant.YES)) {
            gotoState(VoidRefundTrans.State.INPUT_PWD.toString());
        } else if (isNeedInputTransNo) {// need to input trans no.
            gotoState(VoidRefundTrans.State.ENTER_TRANSNO.toString());
    ...
    public void onActionResult(String currentState, ActionResult result) {
        ...
        switch (state) {
            ...
            case ENQUIRE:
                ...
                transData.setTransType(ETransType.VOIDREFUND);
    ...
    private void validateOrigTransData(long origTransNo) {
        ...
        if (!trType.equals(ETransType.REFUND)) {
